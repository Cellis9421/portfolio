...
export const CWindowAboutMe = () => {
  ...
  return (
    <CWindow title="About Calvin Ellis" defaultOpen={isOpen(ABOUT_ME_WINDOW_ID)} onClose={() => close(ABOUT_ME_WINDOW_ID)}>
      <code className="code">
        <div className="p-2 md:p-6 text-xs md:text-sm">
                        ...
                        if (isArray) {
                          return (
                            <span className="color-4">
                              {`[`}
                              <span className="color-1">{value.map((v: any) => `"${v}"`).join(', ')}</span>
                              {`]`}
                            </span>
                          );
                        }
          ...
          <p className="property">
            <Link href="https://github.com/Cellis9421" target="_blank" className="flex group" title="Cellis9421's GitHub Profile">
          ...
          {/** LinkedIn Link */}
          <p className="property">
            <Link href="https://www.linkedin.com/in/calvin-ellis-ma/" target="_blank" className="flex group" title="Cellis9421's LinkedIn Profile">
              <span className="color-2">linkedin</span>
              <span>: </span>
              <span className="flex ml-2 space-x-2">
                <span className="color-4">Calvin Ellis</span>
                <span className="text-editor.comment hidden group-hover:flex ">{'// Connect with me on LinkedIn!'}</span>
              </span>
            </Link>
          </p>
          <span>{`}`}</span>
